...
# Default ENV_NAME 'exp' can be overridden by 'ENV_NAME=foo setup.sh'
ENV_NAME="${ENV_NAME:-exp}"
...
conda env create --name=${ENV_NAME} -f script/exp.yml
...
git clone --depth 1 --recursive https://github.com/pytorch/pytorch \
    "${PYTORCH_SRC}"
pushd "${PYTORCH_SRC}"
TORCH_CUDA_ARCH_LIST="3.0 3.5 5.0 5.2 6.0 6.1 6.1+PTX" MAX_JOBS=12 python setup.py install
popd
TVISION_SRC="${HOME}/.conda/envs/${ENV_NAME}/source/torchvision"
mkdir -p "${TVISION_SRC}"
git clone --depth 1 --recursive https://github.com/pytorch/vision \
    "${TVISION_SRC}"
pushd "${TVISION_SRC}"
TORCH_CUDA_ARCH_LIST="3.0 3.5 5.0 5.2 6.0 6.1 6.1+PTX" MAX_JOBS=12 python setup.py install
popd
